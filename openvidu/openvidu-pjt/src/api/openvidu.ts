...
export const stopRecording = async (recordingId: string): Promise<any> => {
    try {
        const response = await axios.post(`${APPLICATION_SERVER_URL}/recordings/stop/${recordingId}`, {}, { headers: { "Content-Type": "application/json" } });
        return response.data;
    } catch (error) {
...
const detectProfanity = (text: string): boolean => {
    const profanityList = ["녹화", "녹음"]; // Add more words as needed
